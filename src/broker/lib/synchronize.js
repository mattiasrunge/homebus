...
const synchronize = (obj, methodName) => {
    ...
    assert(obj[methodName], `Can not synchronize, no method called ${methodName} found on supplied object`);

    // Define a name for the lock
    const lockName = `__lock_${methodName}`;

    // Make sure synchronize is not called more than once of each method
    assert(!obj[lockName], `Can not synchronize, ${methodName} has already been synchronized`);
